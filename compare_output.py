...
from my_yolov8s.utils import letterbox, non_max_suppression, scale_boxes
...
class COCOEvaluator:
    ...
    def evaluate(self, predictions_dict):
        ...
        for image_id, detections in predictions_dict.items():
            all_predictions.extend(self.convert_predictions_to_coco_format(detections, image_id))
...
class OutputComparator:
    ...
    def compare_single_image(self, img_path, conf_thres=0.25, iou_thres=0.45):
        ...
        # 预处理图像
        img_tensor, _original_img, original_shape = self.preprocess_image(img_path)
        ...
        passed = (
            result["count_match"]
            and result["bbox_iou_avg"] > 0.95
            and result["class_match_rate"] == 1.0
            and result["score_diff_avg"] < 0.001
        )
    ...
    def run_all_tests(self, test_images=None, num_coco_images=100, use_coco_eval=True, verbose=False):
        """运行所有测试.
        ...
        results = {"tensor_test": None, "image_tests": [], "coco_eval": None}
        ...
        if test_images is None:
            if coco_img_dir.exists():
                test_images = sorted(coco_img_dir.glob("*.jpg"))[:num_coco_images]
                print(f"使用 COCO val2017 数据集: {len(test_images)} 张图片")
            else:
                # 回退到内置图片
                assets_dir = Path(__file__).parent / "ultralytics" / "assets"
                test_images = [str(assets_dir / "bus.jpg"), str(assets_dir / "zidane.jpg")]
                print("COCO 目录不存在，使用内置测试图片")
        ...
        if use_coco_eval and coco_anno_file.exists():
            ...
            try:
                ...
                results["coco_eval"] = {"official": official_metrics, "my": my_metrics}
    ...
    def _print_summary(self, results):
        ...
        if total > 0:
            ...
            for r in results["image_tests"]:
                ...
                print(
                    f"   {status} {r['image']}: 检测数 {r['official_count']}/{r['my_count']}, "
...
def main():
    ...
    parser.add_argument("--num-images", type=int, default=100, help="COCO 数据集测试图片数量 (默认: 100)")
    parser.add_argument("--verbose", action="store_true", help="详细输出每张图片结果")
    parser.add_argument("--no-coco-eval", action="store_true", help="禁用 COCO mAP 评估")
    parser.add_argument("--images", type=str, nargs="+", help="指定测试图片路径 (覆盖 COCO)")
